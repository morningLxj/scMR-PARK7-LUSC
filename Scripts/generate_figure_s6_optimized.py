...
# Create figure layout with MORE spacing
# Increase figure width slightly and adjust width ratios
fig = plt.figure(figsize=(14, 6))
gs = gridspec.GridSpec(1, 2, width_ratios=[1, 1.3], wspace=0.4) # Increased wspace from 0.3 to 0.4
...
try:
    ...
    # SHORTEN LABELS: Wrap long text
    # Function to wrap text
    def wrap_labels(text, width=40):
        return "\n".join(textwrap.wrap(text, width))
    
    df_go['Description_Wrapped'] = df_go['Description'].apply(lambda x: wrap_labels(x, width=45))
    ...
    # Adjust layout manually to ensure y-labels fit
    # (tight_layout usually handles this, but we can force margin)
